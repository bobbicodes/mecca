...
(defn bass-clef []
  [:g {:transform "scale(2.3,2.3) translate(2,81.35)"}
  ...
  (let [bassline (subscribe [:bassline])
      ...
      (fn []
        [:svg {:view-box "0 0 480 420"}
         ...
         [bass-clef]
         [treble-clef]
         [:g#staff {:transform "translate(15,50)"
                    :view-box "0 0 450 160"}
          (doall
           (for [y (range 40)]
             ^{:key y}
             [:line {:x1 0 :x2 450 :y1 (* 7 y) :y2 (* 7 y)
                     :stroke "black"
                     :stroke-width (if (= y 18) 0.5 1.05)
                     :stroke-dasharray (if (= y 18) 1)
